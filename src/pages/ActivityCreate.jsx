...
export default function CreateActivities() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
        const res = await fetch(`http://localhost:4000/createactivity/${club_id}`, {
          method: "POST",
          headers: myHeaders,
            ...
            <form onSubmit={handleSubmit}>
              ...
              >
                ...
                <LocalizationProvider dateAdapter={AdapterDayjs} locale={thLocale}>
                    <Grid container item spacing={5} xs={12} sx={{ mb: 2,display:'flex',justifyContent:'center' }}>
                        <Grid item>
                            <DateTimePicker
                                label="เริ่มต้น"
                                value={start_date}
                                ...
                                inputFormat={(value) => dayjs(value).format('DD/MM/YYYY HH:mm')}
                            />
                        </Grid>
                ...
                <Grid item xs={12} sx={{ mb: 2 }}>
                    ...
                    value={activityTypes.find(option => option.activity_type_id === activity_type_id) || null} // Ensure value matches an option
                    onChange={(event, newValue) => {
                        setActivity_type_id(newValue ? newValue.activity_type_id : ''); // Set the selected activity type ID
                    }}
                    renderInput={(params) => (
                        <TextField {...params} label="กิจกรรมประเภท" variant="outlined" />
                    )}
                    />
                </Grid>

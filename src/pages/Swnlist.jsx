...
export default function Page() {
  ...
  const addSWN = async () => {
      try {
        const result = await withReactContent(Swal).fire({
          title: <Typography>เพิ่มรายชื่อศูนย์วิทยบริการและชุมชนสัมพันธ์</Typography>,
          input: 'text',
  ...
  const editSWN = async (swn_id, swn_name) => {
    try {
      const result = await withReactContent(Swal).fire({
        title: <Typography>แก้ไขรายชื่อศูนย์วิทยบริการและชุมชนสัมพันธ์</Typography>,
        input: 'text',
        inputValue: swn_name,
  ...
  const SWNdelete = (swn_id, swn_name) => {
    withReactContent(Swal).fire({
      title: <Typography variant='h6'>ยืนยันว่าจะลบ <br/> {swn_name} ?</Typography>,
      html: (
        <div>
  ...
  return (
    ...
    <Box sx={{
      ...
      <Grid container sx={{ display: 'flex',alignItems:'center' }}>
        ...
        <Grid item  sx={{ pb: 15, justifyContent: "center", display: 'flex' }} xs={12}>
          <Paper elevation={8} sx={{ width: 800, height: 1, background: "#FFF6E1", borderRadius: '0px', display: 'block', pb: 0.5, pt: 0.5 }}>
            {data.map(swn => (
              <Grid key={swn.swn_id}>
                <Button component={Link} to={`/swn/${swn.swn_id}`} sx={{ width: 625, height: 50, fontSize: 20,mb:0.5 }} key={swn.swn_id}>
